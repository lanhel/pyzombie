...
class ExecutablePropertiesTest(ExecutableBase):
    ...
    def runTest(self):
        ...
        self.assertEqual(
            config.get("pyzombie_filesystem", "execbase"), self.ex.execbase
        )
        self.assertEqual(
            config.get("pyzombie_filesystem", "binary"), self.ex.binaryname
        )
        self.assertEqual(self.__class__.__name__, self.ex.name)
        self.assertEqual(
            os.path.join(datadir(), self.__class__.__name__), self.ex.dirpath
        )
        self.assertEqual(
            os.path.join(
                datadir(), self.__class__.__name__, self.ex.binaryname + ".py"
            ),
            self.ex.binpath,
        )
